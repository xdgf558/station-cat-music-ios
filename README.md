...
本仓库目前包含 **M1 工程基础、M2 本地隔离认证、M3 曲库与授权播放、M4 系统播放与队列，以及 M5 个人资料库与设置的本地隔离实现**。可以在模拟器浏览三首明确标注的虚构示例、搜索、持久收藏、打开播放器占位页和切换四种语言。默认仍为 Mock，尚未启用远端登录、真实曲库、音频、个人同步或购买服务；选择示例歌曲不会播放声音。

当前优先交付**不售卖订阅的 v1.0**，订阅阶段暂缓。首版剩余工作见 [发布计划](docs/v1-release-plan.md)，本地缓存和存储预算首批实现见 [收尾报告](docs/v1-storage-report.md)。
...
- M5 新增按环境/账号隔离的持久收藏与最近播放、离线操作日志、冲突处理、历史开关与清除确认；双会话同步只在临时隔离服务验证。详见 [M5 实现、存储决策与验收边界](docs/M5-report.md)。
...
稳定 CI 固定 Xcode **26.4.1**；本机仅安装 Xcode **27 beta 6**，本轮使用该版本验证。M5 PR #7 已合并，其最终审核提交的两套稳定 CI 均通过；真实环境与实体机仍待验收，beta 模拟器结果不能视为真机/上架验收。
...
`Core` 提供可测试的接口和状态基础；`StationCatMusic` 是原生界面；`Tests` 与 `UITests` 验证失败边界及本机交互。个人收藏与最近播放保存在受保护的本机作用域文件中，默认不连接云端；游客数据不会自动并入登录账号。

M2 已实现本地隔离原生会话、PKCE 和账号删除任务恢复；真实 HTTPS / AASA 与真机验证尚未完成，跨产品物理删除仍待保留策略确认。M3 已有隔离授权媒体与原生播放联调，M4 队列/系统控制已有本地实现，M5 已完成隔离个人资料库首批实现；真实 HTTPS、锁屏/后台/耳机与生产同步仍待真实环境及真机验收。StoreKit 音乐包月属于后续阶段，当前没有售价、购买按钮或交易代码。
