...
每组先以 `_exit(73)` 终止测试宿主，再用新的 xcodebuild 测试宿主恢复。驱动同时要求崩溃标记及非零退出，恢复标记及成功测试退出；普通测试跳过这些专用方法。A11/A12 断言两次服务端请求落在原 120 秒窗口内。三组均验证同账号/家族、绝对到期点不变、未误撤销家族，独立删除 receipt 保留且恢复动作仅为查询。
...
Node 24；稳定 CI 固定 Xcode 26.4.1。本机仅有 Xcode 27 beta 6，使用显式 `DEVELOPER_DIR`、`M1_ALLOW_LOCAL_TOOLCHAIN=1`；本机结果不能替代稳定 CI。新增 CI 步骤 checkout 固定后端提交后运行驱动。**开 PR 时先推送配套网站分支，再推送此原生分支，否则 CI 无法取得后端测试来源。** 本轮仅本地验证，尚未运行新增远端 CI。
