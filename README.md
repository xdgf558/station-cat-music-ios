...
本仓库目前是 **M1 工程与契约基础**。可以在模拟器浏览三首明确标注的虚构示例、搜索、临时收藏、打开播放器占位页和切换四种语言。尚未连接真实登录、曲库、音频、个人同步或购买服务；选择示例歌曲不会播放声音。

## 本轮内容

- SwiftUI 原生工程，最低 iOS 18；简体中文、繁体中文、English、日本語；动态字体、语义标签及至少 44pt 操作目标。
- 默认 Mock。Development、Staging、Production 分别写入环境字段，当前全部关闭真实网络。缺失或未知环境也按关闭处理。
- 24 个首版拟定 API 操作、OpenAPI 3.1、请求与响应样例、四语错误键。没有修改旧网站 API，也没有创建这些服务端路由。
- 单一 AVPlayer 所有者、连续时钟到期边界、取消与账号作用域、Keychain 单条记录更新、刷新和删除恢复 journal。
- 本机测试、CI 定义与来源清单。详细边界和结果见 [M1 验收记录](docs/M1-report.md)。
...
稳定 CI 固定 Xcode **26.4.1**；本机仅安装 Xcode **27 beta 6**，本轮使用该版本验证。稳定 CI 尚未远端执行，不能把 beta 模拟器结果视为真机/上架验收。
...
M2 才接入真实原生会话、PKCE 和账号删除服务；M3/M4 接入授权媒体、锁屏与真机播放；M5 做个人数据同步。StoreKit 音乐包月属于后续阶段，M1 没有售价、购买按钮或交易代码。
