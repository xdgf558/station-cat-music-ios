...
## CI 重跑与搜索断言范围（2026-09-18）

原 head 的 PR CI 重跑通过 A11–A13（服务请求间隔 8.519 / 4.844 / 2.694 秒）及推荐/真实音频专项，未重现 A12 仅有 PID 的启动/输出问题；认证恢复实现和 120 秒窗口不变。后续 UI 测试在搜索后的全 App 歌曲存在性断言失败：该查询包含发现页保留的同名歌曲，不能精确代表曲库搜索结果。

给曲库 ScrollView 增加稳定标识，测试只检查此容器内的匹配/不匹配歌曲，并等待不匹配项消失、确认搜索框值为 Night。未放宽成功条件或增加盲目重试。修正后本机 Swift 62 项（6 项专用探针默认跳过）及 UI 2/2 通过；新提交须重新通过稳定 CI 后才能合并。
