...
本仓库目前包含 **M1 工程基础、M2 本地隔离认证，以及 M3 曲库与授权播放、长曲续期稳定性实现**。可以在模拟器浏览三首明确标注的虚构示例、搜索、临时收藏、打开播放器占位页和切换四种语言。默认仍为 Mock，尚未启用远端登录、真实曲库、音频、个人同步或购买服务；选择示例歌曲不会播放声音。
...
稳定 CI 固定 Xcode **26.4.1**；本机仅安装 Xcode **27 beta 6**，本轮使用该版本验证。M1/M2 和 M3 首批合并版本已有稳定 CI 通过记录；本轮长曲稳定性改动尚未推送，仍须按新提交运行稳定 CI，beta 模拟器结果不能视为真机/上架验收。
