...
## 2026-09-19：恢复后服务端证据读取修复

3b3aa42 的 push run 35396437840 在 A12 已完成 RECOVER、FINISHED 和进程退出确认之后，单次五秒 `/fixture/evidence` 请求超时；同提交 PR run 35396442437 通过。失败发生在宿主后置证据读取，现有日志不能确定服务端响应延迟原因。

将读取移入独立 helper，仅对 GET 的网络超时、连接错误与 500/502/503/504 做有限重试：最多四次，二十秒收集预算，单次超时不超过五秒且受剩余预算限制，退避也计入预算，超过预算的成功响应不接受。恢复进程不会重启，不重新发送 refresh、seed 或其他变更请求，不改变固定 120 秒重放窗口。

返回值校验阶段、held、恰好两条请求、200 状态、请求 UUID、结果指纹、generation、未撤销状态、操作数量及对应关系；A11/A12 要求同一请求和结果且间隔小于 120 秒，A13 要求两次不同操作。错误阶段、非法 JSON、超大响应及内容不一致立即失败，不重试。归档只保留已知脱敏字段；摘要记录读取次数与耗时。

新增证据读取回归 10/10 通过，包括真实 loopback 首次 HTTP 读取超时后成功、预算耗尽、错误内容不重试及只执行 GET。原驱动回归 20/20、Python 编译、源码守卫和 OpenAPI 校验通过。用于验证宿主重试预算的模拟时钟不参与认证服务或真实恢复探针。

本机 Xcode 27 beta 6 的真实探针全部通过：A11 PID 16384→16401（请求间隔 0.601 秒）、A12 16416→16432（0.476 秒）、A13 16452→16468（0.499 秒）。本次三个阶段证据均一次读取成功；首次超时场景由独立 HTTP 回归覆盖。未修改产品 Core、音频、Swift 探针或后端 fixture。新提交仍须等待固定 Xcode 26.4.1 CI；以上本机结果不能替代稳定 CI 或实体机验收。

## 后续 P3：完整 HTTP 请求绝对期限

证据请求改为仅允许 loopback 的 HTTPConnection。连接、响应头和响应体共享绝对期限；到期主动 shutdown 底层 socket，finally 关闭连接并等待计时器退出，不留下后台读取。新增真实 HTTP 延迟响应头后延迟响应体、持续滴流测试；11 项证据读取回归通过。此项独立提交，不更改认证重放窗口或产品代码。
